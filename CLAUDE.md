...
- **Design primitives in `src/lib/components/`** — `Button`, `TextInput`, `Field`, `Switch`, `Spinner`, `Badge`, `TierBadge`, `NotionBadge`, `Eyebrow`, `HeroHeading`, `PageHeader`, `AppBar`, `Footer`, `User` (signed-in profile cluster — sibling-parity avatar+pill+Settings+Sign-out), etc. — are DS-styled wrappers/compositions. Reuse them before authoring bespoke styled elements.
