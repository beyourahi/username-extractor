<script lang="ts">
    ...
    // Cloudflare save feedback is inline (no toast) — set in this form's own callbacks.
    let cfSaved = $state(false);
    let cfSaveError = $state("");
                ...
                if (f.message === "Saved") {
                    ...
                    cfSaveError =
                        typeof f.message === "string" && f.message
                            ? f.message
                            : "Couldn't save your Cloudflare settings.";
                    cfSaved = false;
                }
            }
        }
...
>
    ...
    <header class="flex flex-col gap-2.5">
        ...
        <p class={cn(bodyBase, "max-w-prose")}>Defaults, Notion connection, and cleanup tools.</p>
    </header>
            ...
            <form method="POST" action="?/saveCloudflare" use:cfEnhance class="flex flex-col gap-6">
                ...
                <SettingsRow label="Model" htmlFor="cf-model" stacked>
                    <div class="flex min-w-0 items-center gap-2">
                        ...
                        >
                            <RefreshCw class={cn("size-4", refreshingModels && "animate-spin")} aria-hidden="true" />
                        </button>
                        <Select
                            id="cf-model"
                ...
                {:else if cfSaveError}
                    <p class="text-destructive text-caption text-pretty" role="alert">{cfSaveError}</p>
                {/if}
                        ...
                        use:enhance={() =>
                            async ({ result }) => {
                                if (result.type === "success") {
                                    ...
                                    cfSaveError = (result.data?.error as string | undefined) ?? "Couldn't disconnect.";
                                }
                            }}
                        onsubmit={(e) => {
                            if (
